...
def plot_sensor_progression(df, sensor_list, first_peak_cols, peak_cols, sensor_numbers, output_path):
    ...
    cmap = plt.cm.viridis
    colors = cmap(np.linspace(0.0, 0.55, len(df))) 
    ...
    for sensor_id in sensor_list:
        ...
        x_labels.append(f"({sensor_num})")
    ax.set_xticklabels(x_labels, rotation=90, ha='right', fontsize=16)
    ax.set_xlabel('Sensor Number', fontsize=12, fontweight='bold')
...
def plot_sensor_progression_absolute(df, sensor_list, first_peak_cols, peak_cols, sensor_numbers, output_path):
    """Progression plot: X=sensor index, Y=absolute time (wall-clock datetime)"""
    print("\n  Generating absolute-time sensor progression plot...")
    ...
    for idx, row in df.iterrows():
        ...
        # Collect dominant_peak times (absolute, no reference subtraction)
        dominant_peak_times = []
        dominant_peak_indices = []
        ...
        if dominant_peak_times:
            ax.plot(dominant_peak_indices, dominant_peak_times, '--s',
                    color=color, linewidth=1.5, markersize=5,
                    label=f'{vehicle_id}', alpha=0.7)

    # X-axis labels: (sensor_num)
    ax.set_xticks(range(len(sensor_list)))
    x_labels = [f"({sensor_numbers.get(sensor_id, '?')})" for sensor_id in sensor_list]
    ax.set_xticklabels(x_labels, rotation=90, ha='right', fontsize=16)
    ax.set_xlabel('Sensor Number', fontsize=12, fontweight='bold')

    # Y-axis: absolute datetime
    ax.set_ylabel('Absolute Time', fontsize=12, fontweight='bold')
    ax.yaxis.set_major_formatter(mdates.DateFormatter('%H:%M:%S'))
    ax.yaxis.set_major_locator(mdates.AutoDateLocator())

    ax.set_title('Vehicle Progression Through Sensors (Absolute Time)',
    ...
    plt.tight_layout()
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"  ✓ Absolute progression saved: {output_path}")
    plt.close()
...
def main():
    ...
    if args.progression:
        ...
        plot_sensor_progression_absolute(df, sensor_list, first_peak_cols, peak_cols,
                                 sensor_numbers, output_dir / 'vehicle_progression_absolute.png')
...
if __name__ == "__main__":
    ...
    #ex.python3 visualise_cross_sensitivity.py --input res.csv --progression --output_dir set